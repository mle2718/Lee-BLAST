...
*YOU NEED TO EDIT 2 GLOBALS IN VERSION 11
...
*do "/home/mlee/Documents/Workspace/recreational_simulations/cod_and_haddock_GARFO_fy2017/cod_haddock_2017_set11_hadd_stop.do"

*do "/home/mlee/Documents/Workspace/recreational_simulations/cod_and_haddock_GARFO_fy2017/cod_haddock_2017_set14_recal_02_20_2017.do"
...
local rec_out11 "$my_wd/recreational_catches2017_set11.dta"
local rec_out12 "$my_wd/recreational_catches2017_set12.dta"
local rec_out13 "$my_wd/recreational_catches2017_set13.dta"
local rec_out14 "$my_wd/recreational_catches2017_set14.dta"


/*
local rec_out2 "$my_wd/recreational_catches2017_set1a.dta"
local sp1_out1 "$my_wd/cod_end_of_wave2017_sq_regsa.dta"

local sp1_out2 "$my_wd/cod_end_of_wave2017_set1a.dta"
*/


*dsconcat `rec_out1' `rec_out2' `rec_out3' `rec_out5' `rec_out6' `rec_out7' `rec_out8' `rec_out11' `rec_out10' `rec_out12' `rec_out13'
...
export excel scenario total_trips cod_ok haddock_ok cod_dead_pounds cod_mortality_mt haddock_dead_pounds haddock_mortality_mt cod_num_kept cod_num_released haddock_num_kept haddock_num_released cod_pounds_kept cod_pounds_discard haddock_pounds_kept haddock_pounds_discard using "$my_wd/results_2017_exportH.xlsx", replace firstrow( varlabels) sheet("results") 

putexcel set "$my_wd/results_2017_exportH.xlsx", modify sheet(results)
...
export excel using "$my_wd/results_2017_exportH.xlsx",  firstrow(varlabels) sheet("regulations") sheetreplace 
...
replace reg_notes= "Cod closed all year. 12 haddock at 17in" if inlist(scenario,2,102,1002,52,152,1052, 65, 165, 1065, 91, 191, 1091)
replace reg_notes= "Cod closed all year. 10 haddock at 17in" if inlist(scenario,3,103,1003,31,51,131,1031,151,1051, 69, 169, 1069,94, 194, 1094)
replace reg_notes= "Cod closed all year. 8 haddock at 17in" if inlist(scenario,4,104,1004, 85, 185, 1085, 73, 173, 1073, 97,197,1097)
...
replace reg_notes= "Cod closed all year. 12 haddock at 17in.  Haddock closed in Sept" if inlist(scenario,9,109,1009,34,35,134,135,1034,1035, 67, 167, 1067,92, 192, 1092)
replace reg_notes= "Cod closed all year. 12 haddock at 17in.  Haddock closed in October" if inlist(scenario,10,110,1010,36,136,1036, 68, 168, 1068, 93,193,1093)
...
replace reg_notes= "Cod closed all year. 10 haddock at 17in.  Haddock closed in Sept" if inlist(scenario,11,111,1011,32,132,1032, 71, 171, 1071, 95, 195, 1095)
replace reg_notes= "Cod closed all year. 10 haddock at 17in.  Haddock closed in October" if inlist(scenario,12,112,1012,33,133,1033, 72, 172, 1072, 96, 196, 1096)


replace reg_notes= "Cod closed all year. 8 haddock at 17in.  Haddock closed in May." if inlist(scenario,86,186,1086, 74, 174, 1074)
replace reg_notes= "Cod closed all year. 8 haddock at 17in. Haddock closed in Sept" if inlist(scenario,87,187,1087, 75, 175, 1075, 98, 198, 1098)
replace reg_notes= "Cod closed all year. 8 haddock at 17in. Haddock closed in October" if inlist(scenario,88,188,1088, 76, 176, 1076, 99, 199, 1099)
...
replace behavior_notes= "Stop encountering haddock once bag limit is reached. 10 max cod encounters.  50 max haddock" if (scenario>=60 & scenario<=76) | (scenario>=90 & scenario<=99)

replace behavior_notes= "Stop encountering haddock once bag limit is reached. 30 max cod encounters.  50 max haddock" if scenario>=80 & scenario<=90 


replace behavior_notes= "If BOTH cod and haddock are closed,trips do not encounter any cod or haddock." if scenario>=100  & scenario<=199
...
replace  behavior_notes= behavior_notes + " Stop encountering haddock once bag limit is reached. 10 max cod encounters.  50 max haddock." if (scenario>=160 &  scenario<=176) | (scenario>=191 &  scenario<=199 )
replace  behavior_notes= behavior_notes + " Stop encountering haddock once bag limit is reached. 10 max cod encounters.  50 max haddock." if (scenario>=1060 &  scenario<=1076)| ( scenario>=1091 &  scenario<=1099 )
...
*export excel scenario reg_notes behavior_notes using "$my_wd/results_2017_exportH.xlsx",  firstrow(varlabels) sheet("quick_regs") sheetreplace 


! cp "/home/mlee/Documents/Workspace/recreational_simulations/cod_and_haddock_GARFO_fy2017/results_2017_exportH.xlsx" "/run/user/1877/gvfs/smb-share:server=net,share=home2/mlee/dropoff/results_2017_exportH.xlsx"
