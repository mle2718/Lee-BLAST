...
log using "$my_wd/cod_and_haddock_2017_set14.smcl", replace
version 12
...
local econ_out "$my_wd/economic_data2017_set14.dta"
local rec_out "$my_wd/recreational_catches2017_set14.dta"
local sp1_out "$my_wd/cod_end_of_wave2017_set14.dta"
local sp2_out "$my_wd/haddock_end_of_wave2017_set14.dta"
local cod_catch_class "$my_wd/cod_catch_class_dist_2017_set14.dta"
local haddock_catch_class "$my_wd/haddock_catch_class_2017_set14.dta"

local cod_land_class "$my_wd/cod_land_class_dist_2017_set14.dta"
local haddock_land_class "$my_wd/haddock_land_class_2017_set14.dta"
...
global cod_upper_bound 10
global haddock_upper_bound 50
global cmax_age 9
global hmax_age 9
...
global numtrips 259000

global which_year=2016
...
global this_year=2016

 
/* Age-length key years*/
 global lcalibration_start 2013
 global lcalibration_end 2015
...
*/
 global cod_historical_sizeclass "$my_data_dir/cod_size_class_2016_mod.dta"  
 global haddock_historical_sizeclass "$my_data_dir/haddock_size_class_2016_mod.dta" 


 global cod_historical_sizeclass "$my_data_dir/cod_size_class_2016_mod3.dta"  
 global haddock_historical_sizeclass "$my_data_dir/haddock_size_class_2016_mod3.dta" 
 ...
 */
 global cod_naa `""$my_data_dir/cod agepro/cod_naa_2015updatem2.dta""'
 global hadd_naa `""$my_data_dir/haddock agepro/haddock_naa_2015update.dta""'
...
global hadd_naa_start `""$my_data_dir/haddock agepro/2016_HAD_GM_NAA.dta""'
global cod_naa_start `""$my_data_dir/cod agepro/2016_COD_BOTH_NAA.dta""'
...
global cod_catch_class `""$my_data_dir/cod_catch_class_2016_mod3.dta""' 
global haddock_catch_class `""$my_data_dir/haddock_catch_class_2016_mod3.dta""' 

global cod_catch_class `""$my_data_dir/cod_catch_class_2016_mod3.dta""' 
*global haddock_catch_class `""$my_data_dir/haddock_catch_class_2016_mod2.dta""' 
...
global haddock_sublegal_keep=0.05
...
postfile `economic'  scenario month total_trips WTP CV_A CV_E replicate cbag hbag cmin hmin cmax hmax codbag_comply cod_sublegal_keep cod_release_mort hadd_release_mort using `econ_out', replace
postfile `rec_catch' scenario month total_trips cod_num_kept cod_num_released haddock_num_kept haddock_num_released cod_pounds_kept cod_pounds_discard cod_discard_dead_pounds haddock_pounds_kept haddock_pounds_discard haddock_discard_dead_pounds replicate  cbag hbag cmin hmin cmax hmax crep hrep codbag_comply cod_sublegal_keep cod_release_mort hadd_release_mort using `rec_out', replace

postfile `species1' scenario month commercial_catch commercial_discards age1 age2 age3 age4 age5 age6 age7 age8 age9 replicate  cbag hbag cmin hmin cmax hmax cod_release_mort hadd_release_mort using `sp1_out', replace
postfile `species2' scenario month commercial_catch commercial_discards age1 age2 age3 age4 age5 age6 age7 age8 age9 replicate  cbag hbag cmin hmin cmax hmax cod_release_mort hadd_release_mort using `sp2_out', replace
...
gen hadd_max=.
gen fishing_year=.
save "$my_wd/cod_discard_saver.dta", replace
restore
...
global cod2016a=208*.90
global cod2017a=208*.90
global cod2018a=208*.90
...
*/
global haddock2016a=2416*.8
global haddock2017a=2416*.8
global haddock2018a=2416*.8
...
use "$my_wd/haddock_recruits_2015base.dta", clear 
...
use "$my_wd/cod_recruits_2015average.dta"
/* or
...
order id
drop u1 u2
save `""$my_data_dir/cod agepro/cod_beginning_sorted2016.dta""', replace
...
order id
drop u1 u2
save `""$my_data_dir/haddock agepro/haddock_beginning_sorted2016.dta""', replace
...
/**************************************/
/**************************************/
/* YOU NEED TO CONVERT THE WAVE LEVEL REGS TO MONTH LEVEL REGS */
/**************************************/
...
hbag_cy18=J(1,$periods_per_year,15)
...
cmin_cy18=J(1,$periods_per_year,99)
...
/* The Cod in wavees 4 and 5 are slightly off.
We are modeling a closed season for 1 species as a 9 fish limit with a 99" size minimum.
So anglers continue to encounter fish, but do not retain it.*/
...
local scenario_num=200


/* The general strategy is to "reset" the regs and then overwrite them at the beginning of the loop */
foreach had_bag of numlist 12 10 8{

foreach cod_bag of numlist 1{
foreach cod_min of numlist 24{
foreach haddock_month of numlist 0 9 10{

	local ++scenario_num
	local had_min 17

	mata: hbag_cy17=J(1,$periods_per_year,`had_bag')
	mata: hmin_cy17=J(1,$periods_per_year,`had_min')
	mata: hbag_cy18=J(1,$periods_per_year,`had_bag')
	mata: hmin_cy18=J(1,$periods_per_year,`had_min')



	mata: cmin_cy17=J(1,$periods_per_year,99)
	mata: cbag_cy17=J(1,$periods_per_year, 0)

	mata: cmin_cy18=J(1,$periods_per_year,99)
	mata: cbag_cy18=J(1,$periods_per_year, 0)

if `haddock_month'!=0 {
	mata: hbag_cy17[`haddock_month']=`had_bag'
	mata: hmin_cy17[`haddock_month']=99
	mata: hbag_cy18[`haddock_month']=`had_bag'
	mata: hmin_cy18[`haddock_month']=99
	}
else{
}
/* done */
...
mata:haddock_bag_vec=(hbag_cy15,hbag_cy16, hbag_cy17,hbag_cy18)

mata:cod_bag_vec=(cbag_cy15,cbag_cy16,cbag_cy17,cbag_cy18)
mata: haddock_min_vec=(hmin_cy15,hmin_cy16,hmin_cy17,hmin_cy18)

mata: haddock_max_vec=J(1,length(haddock_min_vec),100)

mata: cod_min_vec=(cmin_cy15,cmin_cy16, cmin_cy17,cmin_cy18)
 ...
 *do "historical_normalized_fishing_helper.do"
 do "$my_wd/monthly_historical_normalized_fishing_helper.do"

/*THIS IS THE ENCOUNTERS-PER-TRIP SECTION*/
*do "setup_encounters_per_trip.do"
do "$my_wd/setup_monthly_encounters_per_trip.do"
...
use `""$my_data_dir/haddock agepro/haddock_beginning_sorted2016.dta""', replace
...
save "$my_wd/haddock_age_count.dta", replace
...
/* OPTION 2a:  Draw from the 2013 AGEPRO output, but ensure that the initial conditions are constant across replicates*/
use `""$my_data_dir/cod agepro/cod_beginning_sorted2016.dta""', replace
keep if id==`replicate'
...
save "$my_wd/cod_age_count.dta", replace
...
clear
do "$my_wd/updated_recreational_effort_helper16.do"
...
mata: cod_commercial_catch_monthly=(0 \ 0 \ $cod_quota1*cod_commercial_monthly[.,2] \ $cod_quota2*cod_commercial_monthly[.,2] \ $cod_quota3*cod_commercial_monthly[.,2]\ $cod_quota4*cod_commercial_monthly[.,2]\ $cod_quota5*cod_commercial_monthly[.,2] \ $cod_quota6*cod_commercial_monthly[.,2] \ $cod_quota7*cod_commercial_monthly[.,2] \ $cod_quota8*cod_commercial_monthly[.,2])
mata: haddock_commercial_catch_monthly=(0 \ 0 \  $haddock_quota1*haddock_commercial_monthly[.,2] \ $haddock_quota2*haddock_commercial_monthly[.,2] \ $haddock_quota3*haddock_commercial_monthly[.,2]\ $haddock_quota4*haddock_commercial_monthly[.,2]\ $haddock_quota5*haddock_commercial_monthly[.,2] \ $haddock_quota6*haddock_commercial_monthly[.,2]\ $haddock_quota7*haddock_commercial_monthly[.,2]\ $haddock_quota8*haddock_commercial_monthly[.,2])

mata: cod_commercial_catch_monthly=cod_commercial_catch_monthly[|$comm_month_starter \.|] 
mata: haddock_commercial_catch_monthly=haddock_commercial_catch_monthly[|$comm_month_starter \.|]
...
/* Compute mid-wave stock structure*/
	...
	do "$my_wd/haddock_mortality_helper.do"
	do "$my_wd/cod_mortality_helper.do"
...
save "$my_wd/haddock_length_count.dta", replace
...
merge 1:m age using "$my_wd/cod_smooth_age_length.dta"
foreach var of varlist length*{
	replace `var'=`var'*pre_rec_count
}
collapse (sum)length*
...
save "$my_wd/cod_length_count.dta", replace

/* Recreational Fishing occurs in Feb */
do "$my_wd/simulation_v41_stop_haddock.do"
*quietly do "aux_wtp.do"

quietly do "$my_wd/aux_wtp2.do"
...
do "$my_wd/rolling_age_length_key.do"
...
use  "$my_wd/cod_ages_out.dta", clear
...
use  "$my_wd/haddock_ages_out.dta", clear
foreach var of varlist age*{
...
/* Compute end of period counts and store them in a vector.  These are equivalent to the initial counts for the beginning of the next period (except for wave 6)*/
	do "$my_wd/haddock_end_of_wave_helper.do"
	do "$my_wd/cod_end_of_wave_helper.do"
...
post `economic' (`scenario_num') (`this_month') (scalar(tripcount)) (scalar(total_WTP)) (scalar(total_UA)) (scalar(total_UE))  (`replicate') ($codbag) ($haddockbag) ($cod_min_keep) ($hadd_min_keep) ($cod_max_keep) ($hadd_max_keep) ($pcbag_comply)  ($cod_sublegal_keep)  ($mortality_release) ($haddock_mortality_release)
