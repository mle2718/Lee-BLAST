...
cd "$output_dir"
local store_name "RESULTS_2020.xlsx"
local stub "recreational_catches_2019"
local stub2 "recreational_catches_2020"
local stub3 "recreational_catches_2020Slot"
...
/* the calibrated model produces 49mt of kept cod, 482mt of released cod and 72mt of dead disards.  It should produce 46mt and 104mt respectively*/
/* adjust cod releases and released mortality */

gen cod_released_mt_ADJ=cod_released_mt*104/72
gen cod_released_dead_mt_ADJ=cod_released_dead_mt*104/72


gen cod_mort_mt=cod_kept_mt+cod_released_dead_mt
gen hadd_mort_mt=hadd_kept_mt+hadd_released_dead_mt

gen cod_mort_mt_ADJ=cod_kept_mt+cod_released_dead_mt_ADJ



collapse (sum) cod_tot_cat-hadd_released_dead_mt cod_mort_mt hadd_mort_mt cod_mort_mt_ADJ, by(scenario replicate scenario_num source)
gen cod_ok=0
replace cod_ok=1 if cod_mort_mt<=193

gen cod_ok_ADJ=cod_mort_mt_ADJ<=193

gen hadd_ok=0 
replace hadd_ok=1 if hadd_mort_mt<=2500
pause
...
local graphopts  "over(scenario, label(angle(forty_five))) yline(193, lwidth(thick) lpattern(dash))"

graph box cod_mort_mt, `graphopts'
graph export "cod_box_plots.png", as(png) replace

local graphopts  "over(scenario) yline(193, lwidth(thick) lpattern(dash)) xlabel(,angle(45)) ytitle("Cod Mortality (mt)")"

vioplot cod_mort_mt, `graphopts'
graph export "cod_vio_plots.png", as(png) replace


collapse (p50) total_trips cod_num_kept cod_num_released haddock_num_kept haddock_num_released cod_mort_mt hadd_mort_mt cod_kept_mt cod_released_mt hadd_kept_mt hadd_released_mt cod_avg_weight haddock_relptrip haddock_landptrip cod_landptrip cod_relptrip  cod_mort_mt_ADJ (sum) cod_ok hadd_ok cod_ok_ADJ (count) N=replicate, by(scenario source scenario_num)
replace cod_ok=cod_ok/N*100
replace hadd_ok=hadd_ok/N*100

replace cod_ok_ADJ=cod_ok_ADJ/N*100
...
label var cod_mort_mt_ADJ  "Adjusted Cod Mortality (mt)"
...
label var cod_ok_ADJ "Adjusted % Under Cod subACL"
...
format total %9.0gc
sort scenario_num 
export excel scenario_num scenario total_trips cod_num_kept cod_num_released haddock_num_kept haddock_num_released cod_mort_mt cod_mort_mt_ADJ hadd_mort_mt cod_ok cod_ok_ADJ hadd_ok cod_kept_mt cod_released_mt hadd_kept_mt hadd_released_mt source haddock_relptrip haddock_landptrip cod_landptrip cod_relptrip  using "`store_name'", replace sheet("Summary results") firstrow(varlabels)
