...
renvars, lower;
compress;
save "${source_dir}/cfdbs/monthly_`prefix'_${commercial_grab_start}_${commercial_grab_end}.dta", replace;
...
keep if tag==1;
save "${source_dir}/cfdbs/cod_timing.dta", replace;
save "${source_dir}/cfdbs/haddock_timing.dta", replace;

use "${source_dir}/cfdbs/haddock_timing.dta", clear;
keep if nespp3==147;
...
use "${source_dir}/cfdbs/cod_timing.dta", clear;
keep if nespp3==081;
collapse (sum) live, by(wave);
egen tl=total(live);
gen frac=live/tl;
